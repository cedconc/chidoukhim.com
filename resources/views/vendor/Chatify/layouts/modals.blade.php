  ...
  <div class="app-modal" data-name="delete">
      <div class="app-modal-container">
          <div class="app-modal-card" data-name="delete" data-modal='0'>
              <div class="app-modal-header">{{ __('Confirmer la suppression') }}</div>
              <div class="app-modal-body">{{ __('Vous ne pouvez pas revenir en arrière !') }}</div>
              <div class="app-modal-footer">
                  <a href="javascript:void(0)" class="app-btn cancel">{{ __('Annuler') }}</a>
                  <a href="javascript:void(0)" class="app-btn a-btn-danger delete">{{ __('Supprimer') }}</a>
              </div>
          </div>
      </div>
  </div>
      ...
      <div class="app-modal-container">
          <div class="app-modal-card" data-name="settings" data-modal='0'>
              <form id="updateAvatar" action="{{ route('avatar.update') }}" enctype="multipart/form-data" method="POST">
                  @csrf
                  <div class="app-modal-header">{{ __('Réglages profil') }}</div>
                  <div class="app-modal-body">
                      {{-- Udate profile avatar --}}
                      <div class="avatar av-l upload-avatar-preview"
                      style="background-image: url('{{ assets_base() }}{{ Auth::user()->photo_1 }}');"
                      ></div>
                      <p class="upload-avatar-details"></p>

                      {{-- Dark/Light Mode  --}}
                      <p class="divider"></p>
                      <p class="app-modal-header">{{ __('Mode nuit') }} <span class="
                        {{ Auth::user()->dark_mode > 0 ? 'fas' : 'far' }} fa-moon dark-mode-switch"
                         data-mode="{{ Auth::user()->dark_mode > 0 ? 1 : 0 }}"></span></p>
                      {{-- change messenger color  --}}
                      <p class="divider"></p>
                      <p class="app-modal-header">{{ __('Changer les couleurs de la messagerie') }}</p>
                  ...
                  <div class="app-modal-footer">
                      <a href="javascript:void(0)" class="app-btn cancel">{{ __('Annuler') }}</a>
                      <input type="submit" class="app-btn a-btn-success update" value="{{ __('Valider') }}" />
                  </div>
              </form>
          </div>
      </div>
