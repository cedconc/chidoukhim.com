...
use App\Http\Requests\ChidourAcceptationRequest;
use App\Mail\EmailDeRelanceMessageNonLus;
...
class MessagesController extends Controller
{
    ...
    public function cron_relance_messages_non_lus(){

        $messages = Message::where('seen', 0)->get();

        foreach ($messages as $message){

            $user_from = User::find($message->from_id);
            $user_to = User::find($message->to_id);
            ...
            if( $user_from && $user_to ){
                ...
                if( $user_to->chatrane ){
                    ...
                    $to = $user_to->email;
                    $from = $user_from->prenom . ' ' . $user_from->name;
                    Mail::to($to)->send(new EmailDeRelanceMessageNonLus($from));
                    Log::info("Relance : " . $user_to->email);
    ...
    public function idFetchData(Request $request)
    {
        ...
        return Response::json([
            'favorite' => $favorite,
            'fetch' => $fetch,
            'user_avatar' => assets_base() . $fetch->photo_1,
        ]);
    }
    ...
    public function getContacts(Request $request)
    {
        ...
        // send the response
        return Response::json([
            'contacts' => $users->count() > 0 ? $contacts : '<br><p class="message-hint"><span>' . __("Aucune conversation") . '</span></p>',
        ], 200);
    }
    ...
    public function accept_chidour(ChidourAcceptationRequest $request){

        $destinataire = User::find($request->user_id);

        $chidour = Chidour::where(function($q) use ($destinataire){
            $q->where('origine', $destinataire->id)->orWhere('cible', $destinataire->id);
        })->where(function($q){
            $q->where('origine', Auth::user()->id)->orWhere('cible', Auth::user()->id)->orWhere('chatrane', Auth::user()->id)->orWhere('chatrane_origine', Auth::user()->id);
        })->where('acceptation_cible', 1)->where('acceptation_origine', 1)->first();
        ...
        if( $chidour ){
            ...
            return back()->withSuccess(__('Votre chidouh a été ajouté a votre messagerie !'))->with($data);
        }
    ...
    public function contacter(ChidourAcceptationRequest $request){
    ...
    public function getFavorites(Request $request)
    {
        ...
        return Response::json([
            'favorites' => $favorites->count() > 0
                ? $favoritesList
                : '<p class="message-hint"><span>' . __("Votre liste de chidouhs est vide") . '</span></p>',
        ], 200);
    }
    ...
    public function sharedPhotos(Request $request)
    {
        ...
        // send the response
        return Response::json([
            'shared' => count($shared) > 0 ? $sharedPhotos : '<p class="message-hint"><span>****</span></p>',
        ], 200);
    }
